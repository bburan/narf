...
for ii = 1:length(group_names)
    ...
    % If it is an autogenerated filemask 'zz', treat it uniquely
    % TODO: Now NO KEYS can start with the letters ZZ. THIS WILL EVENTUALLY
    % CAUSE BUGS AND SHOULD BE FIXED. (It probably already has!)
    
    if regexp(g, '^zz')
        only_filecode = {g(3:end)}; 
        ret.(g) = {MODULES.file_masker.mdl(struct('only_filecode', only_filecode))};
        continue;
    end

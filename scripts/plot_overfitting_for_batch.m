function plot_overfitting_for_batch(batch, cellids, modelnames)
% Makes a large overfitting plot
...
for ii = 1:N
    ...
    results = mysql(sql);
    if length(results) ~= N_cells || any(isempty([results.r_fit])) ||any(isempty([results.r_test])) || any(isempty([results.n_parms]))        
        continue;
    else
        data(:, jj, 1) = [results.n_parms];
        data(:, jj, 2) = [results.r_test] - [results.r_fit];    
        modelnames{jj} = models{ii};
        jj = jj + 1;
    end
end

plot_complexity(data, modelnames, 'r_test - r_fit');

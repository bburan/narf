function ret = mean_sqrt_downsample(args)
% Default Parameters
params = [];
params.raw_freq = 100000;
params.ds_freq = 200;
params.ds_dim_num = 2;  % The dimension on which to downsample
params.pretty_name = 'mean(sqrt(abs(stim))), binned resp';
...
% ------------------------------------------------------------------------
% INSTANCE METHODS
function [ds_stim, ds_resp] = do_mean_sqrt_downsample(parm, stim, resp)
    scale = ceil(parm.raw_freq / parm.ds_freq);
    ds_stim=conv_fn(sqrt(abs(stim)), parm.ds_dim_num, @mean, scale, 0);
    ds_resp=conv_fn(resp, parm.ds_dim_num, @(x) (parm.ds_freq * mean(x)), scale, 0);
end

% ------------------------------------------------------------------------
% Put the instance methods in params struct
params.downsamp_fn = @do_mean_sqrt_downsample;
ret = params;
end
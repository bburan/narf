...
function x = do_depression_filter(mdl, x)
    ...
    if isfield(mdl,'per_channel') && mdl.per_channel,
        num_channels=size(mdl.tau,2)./N;
    else
        num_channels=size(mdl.tau,2);
    end
        ...
        for s = 1:S
            ...
            if mdl.per_channel
                ...
                if mdl.crosstalk && size(stim_in,1)>1,
                    ...
                    for jj=1:N,
                        ui=(1./stimmax(jj))*mdl.strength(jj)./100;
                        taui=mdl.tau(jj).*raw_stim_fs/mdl.tau_norm;
                        tresp=depression_bank(stim_in(jj,:),ui,taui,...
                                              1,ctstim(jj,:));
                        depresp=cat(1,depresp,tresp);
                    end
                else
                    for jj=1:N,
                        rr=(jj-1)*num_channels+(1:num_channels);
                        ui=(1./stimmax(jj))*mdl.strength(:,rr)./100;
                        taui=mdl.tau(:,rr).*raw_stim_fs/mdl.tau_norm;
                        tresp=adp_bank(stim_in(jj,:),ui,taui,1);
                        depresp=cat(1,depresp,tresp);
                    end
                end
            else
                ui=(1./stimmax(:))*mdl.strength./100;
                taui=mdl.tau.*raw_stim_fs/mdl.tau_norm;
                depresp=depression_bank(stim_in,ui,taui,1,mdl.crosstalk);
            end
...
function do_depression_cartoon(sel, stack, xxx)    
    ...
    proc_count=size(mdl.tau,1);
    syn_count=size(mdl.tau,2);
    ...
    if mdl.per_channel,
        x.dat.demo.(mdl.input)=repmat(x.dat.demo.(mdl.input),[1 1 syn_count]);
    end
    ...
    for jj=1:length(mdls),
        ...
        data=cat(1,data,[x.dat.demo.(mdls{jj}.input)(:,1)+1 ...
    ...
    if size(data,2)>3,
        ...
        for jj=1:length(mdls),
            ...
            legstr={};
            for ii=1:syn_count,
                mdls{jj}.strength(1,ii)
                text(1.55+(jj-1)*3.5,ii,...
                     sprintf('%2d:u=%.1f,{\\tau}=%.0f',...
                             ii,mdls{jj}.strength(1,ii),...
                             abs(mdls{jj}.tau(1,ii)./mdls{jj}.tau_norm.*...
                                 1000)),'Color',[1 1 1]);
            end
        end
    else
        ...
        for jj=1:length(mdls),
            ...
            for ii=1:length(mdls{jj}.tau);
                legstr{ii}=sprintf('%2d:u=%.1f,{\\tau}=%.0f',...
                        ii,mdls{jj}.strength(1,ii),...
                        abs(mdls{jj}.tau(1,ii)./mdls{jj}.tau_norm.*1000));
            end

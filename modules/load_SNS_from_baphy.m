function m = load_SNS_from_baphy(args)
...
m = [];
m.mdl = @load_SNS_from_baphy;
m.name = 'load_SNS_from_baphy';
m.fn = @do_load_SNS_from_baphy;
m.pretty_name = 'Load stim+resp for SNS data';
...
m.plot_fns{3}.fn = @do_plot_channels_as_heatmap;%(xx, stck, m.output_stim);
m.plot_fns{3}.pretty_name = 'Stim Channels (Heatmap)';
m.plot_fns{4}.fn = @do_plot_respavg;
m.plot_fns{4}.pretty_name = 'Response Average';
m.plot_fns{5}.fn = @do_plot_response_raster;
m.plot_fns{5}.pretty_name = 'Response Raster';
...
function x = do_load_SNS_from_baphy(mdl, x, stack, xxx)
    ...
    for f_idx = 1:len;
        ...
        % load the stim and response files
        stimfile = [cfd(idx).stimpath cfd(idx).stimfile];%parmfile
        respfile = [cfd(idx).path cfd(idx).respfile];%spikefile
        fprintf('Loading stimulus: %s\n', stimfile);
       
        %record a bunch of options to load response
        % if loadbytrial,
        options=[];
        % commented out -- old options (irrelevant?)
        ...
        %to accomodate evalin parameters in LoadMFile
        globalparams=[];
        exptparams=[];
        exptevents=[];
        TTT_oldbase=[];
        ...
        [resp,tags,trialset,exptevents,sortextras]= ...
            loadspikeraster(respfile,options);
        
        %load the stimulus spectrogram
        options.filtfmt='specgram';
        options.chancount=mdl.stimulus_channel_count;
        options.filtfmt=mdl.stimulus_format;
        stim=loadstimbytrial(stimfile,options);
        ...
        params.Sequences=cell(params.TrialCount,1);
        for trialidx=1:params.TrialCount,
            ...
            for ee=1:length(ev),
                ...
                tnote=tnote{2};
                v=strsep(tnote,'+');
                params.Sequences{trialidx}(ee,1:length(v))=cat(2,v{:});
            end
        end
        ...
        params.CellCount=size(resp,3);
        
        % trim stray bins from end of each trial
        resp=resp(1:params.TrialBins,:,:,:);
        stim=stim(:,1:params.TrialBins,:,:);

        % trim all-zero trials (for funny crash conditions/cell loss)
        zerocheck=nansum(nansum(resp,1),3);
...
%------- just use the combined spectrogram for stim
       % this might not be what Stephen wants
       % it makes it channel x time x trial

        stim = stim(:,:,:,3);
               
        %just want a certain type of trial
        trial_type=6;
        tt=find(ismember(params.BigCat,trial_type));
        resp=resp(:,tt,:);
        stim=stim(:,:,tt,:);
        ...
        if strcmp(mdl.stimulus_format, 'envelope') ||...
            ...
            % narf want to be in format time x stimulus x channel
        ...
        % destroyed our ability to sample stim and resp at
        % different rates
        ...
        % est and val determination
        % if specified, pull out either estimation (fit) or
        % validation (test) subset of the data
        if datasubset,
            repcount=squeeze(sum(~isnan(resp(1,:,:)),3));
            %if max(repcount)>2,
...
function do_plot_channels_as_heatmap(sel, stack, xxx)
    ...
    %[mdls, xins, xouts] = calc_paramsets(stack, xxx(1:end-1));  
        ...
        h = imagesc(xouts{ii}.dat.(sel.stimfile).(mdls{1}.output_stim_time)(:),...
                    1:size(xouts{ii}.dat.(sel.stimfile).(mdls{1}.output_stim),3),...
                    squeeze(xouts{ii}.dat.(sel.stimfile).(mdls{1} ...
                                                          .output_stim)(:, ...
                                                          sel.stim_idx, :))');
        do_xlabel('Time [s]');
        do_ylabel('Output [-]');

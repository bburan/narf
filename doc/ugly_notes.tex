...
We are modeling our neurons with a FIR filter, which is a linear, stationary model. Is that really true? By grouping neurons according to the time since the previous spike, we should be able to divide a dataset into multiple groups (paramsets). If the FIR filters trained on each of these datasets differs significantly, it implies that the model is not stationary. Such FIR filters may also hint at a simple mechanism which we can use to model the time-varying dynamics of the neuron.
...
Smooth Respavg (SMRAV keyword) appears to help improve fits at 200Hz. INFERP did not appear to help at all. {'env200', {'null', 'smrav', 'inferp', 'inferp2', 'inferp4', 'inferp6', 'inferp10', 'inferp20'}, 'logfree', 'firno16', 'siglog', 'mixfit0'}

* Current Experiment on Batch 179 (SPN single channel)  
...
* Current Best Idea: 
  Feature extractors (All-Pole Gammatone filter), sigmoidal nonlinearities functions (compressor) forms input layer. 
  Second layer of network is the linear filter and output nonlinearity.

* Idea:
  When we use gammatones, we find the amplitude at a particular frequency. 
  However, it's probably just as important to find the amplitude at a variety of frequencies. 
  Probably, using a wide frequency filter as well as a notch filter will help model neural activity.


** Per-module memoization
   + Could be done by memoizing calc_xxx() or add_module(), as appropriate
   + Would speed up 
   + Reasons why it would be nice to do this:
     1. narf_modelpane could go very fast, so inspections would become really quick. 
   + Reasons why I don't want to do this:
     1. It would entail storing additional megs per model file, and would have to be run ONCE, AFTER the fitting, to create the cached files. 
     2. If we change a module later, do we want to load the saved version or the new one?
     3. We could achieve much the same functionality by asking load_model and save_model to store XXX anyway, instead of re-building it. 
   + Problem workaround:
     1. I think I will just memoize INSIDE the gt100 module. 

** Memoization Clearing
   + I don't know if I like the idea of managing this by hand. It doesn't really hurt anything, though. 


Q: Which starting features yield the best performance for a simple compressor+FIR model? Does removing the prestim silence improve model performance?
A: Removing prestim silence does not help performance. {{'env100', 'env100n'}, 'log2b', 'firno', 'initrc', 'nonl', 'mse', 'boost'}
...
** Expected Problem
   There are MANY more samples at the 0 (prestim silence) input level...and residual is often a function of input amplitude, so this biases the answer?

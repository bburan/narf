% function fit05ls()
%
% just like fit05 but maximize log-likelihood rather than min MSE
% and apply shrinkage to ll measurement.
%
function fit05ls()
...
function [a,b,c,d] = step_until_10neg2(prev_opts)              
...
function [a,b,c,d] = step_until_10neg65(prev_opts)
    if exist('prev_opts', 'var')
        [a,b,c,d] = fit_boo(prev_opts);
    else
        [a,b,c,d] = fit_boo('StopAtAbsScoreDelta', 10^-6.5, ...
                            'StopAtStepNumber', MaxStepsPerIteration, ...
                            'StepGrowth', StepGrowth);
    end
end
...
%fit_iteratively(@step_until_10neg2, ...
%                create_term_fn());
...
fit_iteratively(@step_until_10neg65, ...
                create_term_fn());

2. Use low-parameter models to characterize spectro-temporal encoding of vocalizations in controlled behavioral states, experiments for which data are often limited.

* Ask SVD
  - Can we have stimuli that are not just vocalizations, but really a variety of natural sounds?
* TUESDAY
  1. [ ] Make a bit plotting function using log-likelihood data
  2. [ ] Make a module that computes log-likelihood in the simple gaussian way. 
...
* IMPORTANT REALIZATION WHILE DISCUSSING WITH DAN
...
* Work Queue
  1. [ ] Run the best PZ models on 259, and then later on 240 and verify PZ model conclusions.
  ...
  4. [ ] Module: Take the FFT of a spike train, then filter out frq components above some threshold to make a smoothed RESPAVG.
  5. [ ] Verify that the fittable gammatones use Hilbert Envelope correctly
  6. [ ] Try a siglog parameterized like page 374 of bayesian book
  7. [ ] Fitter: Boosting with param size scaling (Take variance between current point and slight deviation. Take a VERY SMALL deviation (10^-6, at first. If that's too small, try factors of 10)
  8. [ ] Metric: Log-likelihood
  9. [ ] Metric: NMSE in DB
  10. [ ] Metric: Classifer model (spike = 1, nospike = -1, error function = (1-r_hat*r)^2
  11. [ ] Fitter: Not using an input channel to each module on some steps. (NAN out a chan?)
  12. [ ] Fitter: skipping some model parameters each step.
  13. [ ] END PLOT: Heatmap + raster of resp
  14. [ ] Plot R/sigma^2 vs spike isolation level 
  15. [ ] Plot noise ceiling vs spike isolation
  16. [ ] Plot # params vs avg performance
  18. [ ] Try different initial conditions for NIM models?
  19. [ ] Compare a slow pole vs depression vs other depression ideas
  20. [ ] Optimize the Pole-Zero models so that they run faster
  21. [ ] Ideas for depression or gain control?
  22. [ ] Show that "delayed inhibition" effect is an aliasing of something else.
  23. [ ] Start using XXX to transmit time-varying gain levels or parameters.
  24. [ ] Try different parameterizations of existing, successful modules. 
...
* Clustering
  God, it would be nice if I could peek on results that aren't complete yet.
  Or, if when MATLAB "crashes", it dumps the state so that you can recover all the work that was done.

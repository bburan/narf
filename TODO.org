* MILESTONE
  Some of the optimization is clearly NOT part of the model: sampling method and termination condition (Since you want to plot how the error is decreasing)
  Some of the optimization COULD be part of the model: Performance metric (since you want to plot the prediction)
  But I think those are both bad ideas since they contaminate the model itself.
  Remove Load Above Files button and use callback to initialize model structure
  Give the STACK (and XXX?) to the model to help it initialize?
  ...
  Another problem: Make a STACK, create a GUI for it, close the GUI, try to recreate...watch the failures occur! I guess GUI should be separate from the STACK somehow?
  ...
  Write a programmatic demo for loading a model which fit using TORC data to produce an STRF

* Comments from Stephen
  - Stephen will do the init condition for FIRN coefs split into two filters of positive/negative coefs only    
  - Write a termination condition that ends when "delta = 10^-5 * max-delta-found-so-far" for boosting
  - Profile the time spent during boosting and look for optimizations:

* TODAY
  1. [ ] NSF proposal tweaks.
  2. [ ] Talk with Henry about setting up the cluster. 
  3. [ ] Sparse NMSE penalty 
  4. [ ] Noise Ceiling (Theunisson and Hsu paper, cross correlation)
  5. [ ] Allow Per file normalization (add fitter and splitter)
  6. [ ] Try time-varying stimulus idea. 
...
* Ideas:
  ...
  - FIR filter with a gain term, so that all FIR coefficients are relative to that.
    Or even, for ALL of the terms, having a scaling term like that. 
  ...
  - When put into ABCD form, can any of the matrices be decomposed?
...
* Queue Questions
  - Sparsity penalty trials queued 
...
* Larger Objectives
  1. Fancy early stop criterion or Sparseness Penalty 
  2. STOP BINNING RESPONSEAVG and instead come up with a correlation that uses individual spike times to record instantaneous 
  3. Code up the Hsu & Theunissen noise-ceiling corrected prediction score

* Next Steps:
  1. Read Hsu, Theunisson and try to understand how much information is in the response data. 
  2. Still need to fix problems with parameter scaling (0.1 vs 1000) and normalizing by effect variance
  3. Is there a way to weight data points by NOVELTY? We want to discard outliers, but also don't want to weight repetitive signals so strongly that we overfit to the repetitive signals and not the new data.
  4. Mutual Information fits queued up
  5. Add a two-model comparison function which scatter-plots many matrices, does a histogram of collapsed scatter plot along diagonal, and does randomized paired T-test. Maybe log-likelihood ratio too. 
  6. Read Dan Butz NIM paper and package
  7. Add a button to LOADSTIMFROMBAPHY that plays the sound or stimulus selected
  8. Write a generalized hook-in for constraints that pack_fittables calls and resets values as they are re-inserted
  9. General purpose scatter + marginals plot (Reuse scatter comparison?)
  10. Check that all the fit_* stuff still works,
  11. Roll fit_splits together

...
* Irregularities that need fixing
  ...
  - Jackshrink needs to be tested and used in some keyword functions

* Actions
  1. [-] Repair Fitters 
	 - [-] Abstract functions to be used by every fitter
	       - [ ] Remove, then re add test_set data
	       - [X] Default print_progress () routine
	       - [X] Use of the META.performance_metric() function
	       - [X] Meaningful code for what the fitter returned?
  ...
  3. [-] Refactor
	 ...
	 - [X] Pull out repeated code blocks in the fitters (because they are all pretty much the same damn thing over and over again)
  4. [ ] Default per-paramset, per-channel heatmap graph fns:
	 - [ ] Loadstimfrompbaphy
	 - [ ] Nonrmalize channels
	 - [ ] FIR filter
	 - [ ] Nonlinearity
  5. [ ] Add new functionality to the do_scatter_plot method

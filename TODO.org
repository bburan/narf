...
* TODO: 
  Check that all the fitters still work,
  Roll fit_splits together
...
** Expected Problem
   There are MANY more samples at the 0 (prestim silence) input level...and residual is often a function of input amplitude.
     
* The New Basic Fitter Interface Rules
...
* Fitting Use Cases
  1. Quick fits (for initial conditions, testing, or iterative fits)
     - Take N steps
     - Take steps for 1-5 minutes
     - Take steps until efficiency drops to 1% of first five improvements.
  2. Efficiency tests (for comparing fitters)
     - Take steps for 30 minutes and see overall performance (all machines being equal)
     - Take steps until objfn() has been evaluated 10000 times.
  3. Relative stepsize case (for when your metric has no special meaning or when its scale varies from cell to cell)
     - Avoid whenever possible, since it makes termination conditions much more sensitive to initial conditions. 
     - Take steps until efficiency drops to 0.1% or 0.01% of first five improvements.
  4. Absolute stepsize case (for when your metric has a meaning, such as fraction of variance explained)
     - Take steps until your fraction of variance is increasing by less than 10^-6 each step.
  5. Iterative or SCAAT fits (for harder fits)
     - Inner loops are as above (Usually N steps or absolute improvement small, not relative)
     - Outer loop continues until:
       1) No steps were taken in any module
       2) When the total improvement over all modules is less than some absolute amount (relative?) 

* Stephen's Discussion Points
  1) Does NMSE work as a performance metric?
     Yes. Just use the nmse() keyword and things should work fine. 

  2) Do scatter plots show depfree always beats firno with mixfit3, mixfit4, or mixfit5 for batches 240 and 251?
     Nope. 
     
  3) Which is a good 4 minute initial condition boost routine?
     Redo this.
     
  4) What are the optimal number of boost steps and iterations during iterative fitting?
     I'm not sure how to answer this in general, since it probably depends upon model structure. 

  5) What level of relative stepsize helps?
     I don't know. For batch 251, I was able to get relative stepsizes to work a little better (although not in the first 2 minutes of initialization), but not at all for 240. 

  6) Is there a way to use correlation as a stop criterion? 
     If in the last 100 steps, we have not improved the minimum correlation seen so far by more than 0.001, stop. 
     
* TODAYS WORK
  - [X] Normalized mean squared error
  - [ ] Termination condition for the outer loop
  - [ ] Add a button to LOADSTIMFROMBAPHY that plays the sound or stimulus selected
...
* Next Steps:
  ...
  2. Write a generalized hook-in for constraints that pack_fittables calls and resets values as they are re-inserted
  ...
  7. Modelstring bug.
  8. Make split modules visible/editable from the GUI
  9. Mixfit variations explored more.
  10. Queue up a few different smooth_respavg kernels at 200Hz  
  11. A plot of # of free parameters (put in database)
  12. Profile code and look for optimizations that would speed up each iteration
  13. Why does SENL drop to keyboard?
  14. Heatmaps: loadstimfrombaphy, FIR, nonlinearity
  15. Scatter plots of two models being compared. One plot for each performance metric.
  16. Updated DB Insertion and Great Name Replacing proposal
  ...
  17. Better initial conditions.
  18. Outlier rejection. Exclude worst 1% in MSE calculations? Start at L2, gradually switch to L1 as time goes on? Remove each data point and use fitdist() to estimate it's effect on the noise model probability. Sort these effects. Mark the n% worst data points for plotting as "outliers" and also for removal during MSE or CORR calculations.
  19. Narf Batch categories. 

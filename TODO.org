...
* TODAY
  1. [X] Remove Training set during fitting, then re-add it (fit single model)
  2. [X] Computer Access Administration
  3. [ ] Queue up mixfit3 (which is like mixfit1, but modified to be closer to qboosto0?)
  4. [ ] Test fitters for gt12ch_logfree_firno_initrc_siglogIC
  5. [ ] Queue up a few different smooth_respavg kernels?

* TODO:
  - SURVEYS CHECKLIST:
     - [X] Is batch 240 refreshed?
     - [X] Has all code been pushed?
     - [ ] Fitters: Add mixfit0 to the mix?
     - [ ] Compressors: Remove initrc and run with boost0
     - [ ] Depression: Rerun with boost0. 
     - [ ] Filters: Consolidate IFN crap, do with boost0
     - [ ] Interaction: Try best compressors, nonlinearities, boost0
  - [ ] When the results of the fitters come back, possibly alter the interaction survey
  - Plots next time
    - Scatter plots of fitters
    - Scatter plots of best compressors
    - Scatter plots of best filters (depfree vs firno)
    - Scatter plots of other crap. 
  - 
* Avenues of Research
  1. Grant: a way to pay for myself and get a grant to my name, but stay in this lab?
     ( Look at NRSA grant from NIH, due in December, called the Kirchstein award, F-32. I would be a good applicant, and could probably modify SVD's grant and list my own publication.  Also, a hearing health foundation grant might also help. )
  2. Compare Boost algorithms efficiency Per step, over time. Do more variations.       
  3. Metrics:
     Add BIC, AIC, mutual information, etc
  4. Surveys:
     1) Do a survey with FIR2NP_INIT10_IRNR? 
     2) Batch 242? 
     3) Batch 246 and spectrally complex data?
     4) Batch 240 at 200Hz?
  5. Population parameter histograms.
     1) Fit times vs fitter
  6. Outlier rejection:
     1) Exclude worst 1% in MSE calculations?
     2) Start at L2, gradually switch to L1 as time goes on?
  7. Improved initial conditions:
     1) Depression
     2) FIR filters
  8. By-Hand Observations
  9. Additional keywords
     1) Simple-Delay FIR
     2) Dep2chfree
     3) boost5 (Add lsq to end of boosto4)
     4) Check poly2, poly3 for zero offsets
  10. Additional modules
      1) Gain changing module
  11. Engineering fixes
      2) Modelstring bug
  12. Filter properties as they change over time
      1) Split according to time in file
      2) Or, just look at autocorrelation (leaky integrator)
  13. Optimization
      1) Faster fitting by masking out verification data set
      2) Profiling to understand what is taking the longest in our "best" model

* HIGH LEVEL GOALS:
  2. Make vocalization data (246) fits take < 1 day for IFN models
  ...
  5. Make a new "Narf Batch Category" containing relevant cells for final analysis so they are more easily grouped

* NEXT
  - [ ] Modelstring bug {'env100', 'log2b', {'firno', 'firn', 'fir2n', 'firn2np', 'irn', 'irnr', 'firnpr', 'firnv2', 'depn', 'depno', 'depped', {{'inex', 'init10'}}}, 'nonl', 'mse', 'boost'}
  - [ ] Scripts	
    - [ ] Apply a function to every model file selected
    - [ ] Create a new DB cache with all metrics, fit times
    - [ ] Runscript on 240 to create avg fit time plot
...
* Engineering
  ...
  2. [ ] Improve smoothed nonlinearity plots with a kernel smooth instead of binning
  3. [ ] Reduce, Plot function abstractions for working with STACKs of models
  4. [ ] After saving the model, but before database insertion, do other metrics
  5. [ ] Optimization: If a keyword string already exists, load that rather than refit all the way to that point....
  6. [ ] N Scatter plots of two models being compared, where N is the scatter plot for each performance metric.
  7. [ ] Correlation score that somehow discards outliers? MSE that discards outliers?
  ...
  12. [ ] Safety check so that these modules do not allow unifiers/splitters
          - Correlation, MSE, bayesian, loader, etc
	  - Should normalization module do splitting or not?
  13. [ ] Repair Fitters 
          ...
          - [ ] Bugfix: when using init0, siglog/nonl doesn't work
  ...
  15. [ ] Profile and look for optimizations:
...
* Experiments:
  1. [ ] Really Simple Models
	 Logfit (1 parm) + scale & delay amount per channel (2*n parms) + respavg-estimated stateful depression amount (2*n parms)
  2. [ ] I still think a stateful, low-order ABCD model with nonlinear scalar linkages and a thresholded state reset is the best way to do things!
  ...
  8. [ ] Possible features to extract (And what what is needed to detect them)
  ...
  10. [ ] ABCD Control Blocks with arbitrary functions (start with 1st and second degree polynomials)
  ...
  16. [ ] Is it better to bin, reload from baphy, resample, downsaple, decimate, convolve, smooth, or infer respavg?

* Queue Status
  - All surveys requeued using fit00, fit01, fit02, fit03, fit04. 
...
* Next Steps:
  1. Discover why fit02 has subtly different results than the other fits for the FIR only model.   
  ...
  4. Read Dan Butz paper
  5. Add a button to LOADSTIMFROMBAPHY that plays the sound or stimulus selected
  6. Write a generalized hook-in for constraints that pack_fittables calls and resets values as they are re-inserted
  7. General purpose scatter + marginals plot (Reuse scatter comparison?)
  8. Check that all the fit_* stuff still works,
  9. Roll fit_splits together

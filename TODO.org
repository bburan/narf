* NEXT STEPS:
  1. [ ] Train a model on five data sets, then let a parameter be free for each data set and train again. 
	 Record and display the changes for each model trained on each data set.
  2. [ ] fminval investigation to use an arbitrary evaluation function

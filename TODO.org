* ROADMAP
  1. [X] Report on results of sparsity and new nl comparisons comparison across 240, 242
  2. [X] Try the Sparse bayes fitter
  3. [X] Fix the NL plots
  4. [ ] If empty test set, use 'training set' data for heat map plots
  5. [ ] Choose a 'best on average' amount of sparseness penalty
  6. [ ] Make a new summary for cellids, with HTML data table output with integrated images instead of 5 different plots.
  7. [ ] Input-spanning depression bank. 
  8. [ ] Start leveraging Stephen's job distribution code
  9. [ ] Cleaning, bugfixes, and user interface changes that have accumulated and are burying me
  10. [ ] Start using BAPHY for caching analysis results

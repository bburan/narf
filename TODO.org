...
* Things to try after last discussion with Stephen
  1) Experiment: Does NMSE work as a performance metric?
     Yes. Results for 4 minute fits with boo12 (MSE) vs boo14(NMSE) are different. 

  2) Experiment: Do scatter plots show depfree always beats firno with mixfit3, mixfit4, or mixfit5
     
  3) Experiment: Which is a good 4 minute initial condition boost routine?
     
  4) Experiment: What are the optimal number of boost steps and iterations during iterative fitting?

  5) Experiment: What level of relative stepsize helps?
     Answer: I don't know? For batch 251, I was able to get relative stepsizes to work better (although not in the first 2 minutes), but not for 240. 

  7) Experiment: Is there a way to use correlation as a stop criterion? 
     If in the last 100 steps, we have not improved the minimum correlation more than 0.001, stop. 

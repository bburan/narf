* HIGH LEVEL GOALS:
  1. Somehow, make vocalization data (246) fits take < 1 day for IFN models.
     - Boosting algorithm correction to lessen n^2 performance scaling
       ...
       2) Nested boosting by 
       3) Keep a list of the top N coefs. 9 out of 10 times, boost only on those.
       4) Can I group coefs into small N and boost on them?
	  - Boosting too long on a subset loses plausibility
	  - Heirarchical boosting
	    - Group into sets of size N = sqrt(n). 
	    - Boost through each set. 
...
* Queued Experiments
  1. Does boostit work as well as we hope?
  2. Does normalization after BAPHY help?
  3. How do the NPNLs fare?

* Priorities
  ...
  2. The three initial conditions: 
     1) [X] All zeros
     2) [ ] RC correlation, but with one channel zeroed.
     3) [ ] With the FIRN coefs split into two filters of positive/negative coefs only    

* Actions
  1. [-] Repair NarfModelpane 
	 - [X] Add the FILECODE to the baphy plot module so it is visible to the user
	 - [X] Widen the module plot selectors so you can read long respfile names
         - [X] initzero needs its own keyword
         - [X] Displays META info model name, training set, test set, and other important info at bottom of window
	 - [ ] Repair the add/delete module button functionality
	 - [ ] What module does the GUI edit anymore?
         - [ ] IRRITATION: Why can't I resize windows? 
         - [ ] Invalidate data BELOW the present point on a table-edit callback... and to update the gui to reflect this!
  3. [ ] Repair Fitters 
	 - [ ] Fitters need access to the penalty, which should be stored in the META struct or in the FITTER itself
	       Actually, instead of a 'penalty' and a datastructure, just attach a function that computes the penalty instead	 
	       We also need a 'getsparsity()' method for the META struct
	 - [ ] Provide a dropdown to pick more fitters
	 - [ ] Make the fits run faster by removing then re-adding the 'test_set' data after fitting process  
	 - [ ] Make sure that fitters return meaningful, specific codes indicating how they terminated
  4. [ ] Repair Narf Browser
	 - [ ] Sparsity is not going into the DB
	 - [ ] Antialiasing problem when saving images
	 - [ ] Heat Map current display button in NARF
	 - [ ] AND/OR/NOT query token filter
  ...
  5. [ ] Refactor
	 ...
	 - [ ] Pull out repeated code blocks in the fitters (because they are all pretty much the same damn thing over and over again)
  6. [ ] Default per-paramset, per-channel heatmap graph fns:
	 - [ ] Loadstimfrompbaphy
	 - [ ] Nonrmalize channels
	 - [ ] FIR filter
	 - [ ] Nonlinearity
  7. [ ] Add new functionality to the do_scatter_plot method

* Actions
  ...
  3. [ ] Default per-paramset, per-channel heatmap graph fns:
	 ...
	 - [ ] FIR filter
	 - [ ] Nonlinearity
  4. [ ] Scatter plots with distributions
  5. [ ] Add functionality
  ...
  10. [ ] jackknifing needs to work with other performance metrics?
  11. [ ] Profile the time spent during boosting and look for optimizations:
...
* ENQUEUING MANAGER PROGRAM:
  1. Avoiding Bugs
     I've noticed the following bugs in our job queueing system so far.
      1. Stephen and I have queued up the same work and end up overwriting each other redundantly
      2. I forget to git push to my workers before starting the jobs
      3. I edit my own code while the cluster is running, thus messes up just the jobs that run on my own machine.
  2. Should Batching should work like this?

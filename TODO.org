...
** (#5) Nonlinearities
   Siglog100 wins. But sigrich does very well and is slightly cheaper.
...
** Conclusions
  ...
  - Bar chart with mean/deviation

* Sure would be nice if...
  The number of free parameters was stored in the DB!
  The "ADD ANALYSIS" button still worked
  There was a "DIRECT TO PRETTY FIGURE" plotter so figures were easier to make
  There was a "publish" button that wrote a paper.
  Elliptic Bandpass Filterbank was just Filterbank
* TO DO
  1. [ ] Fit14: SCAAT fitting that starts out with a stepsize equal to the current parameter size
	 Also, try to find the failure point in FIT10 relative to FIT05
  2. [ ] Quadratic terms added via a tensor-type factorization
  3. [ ] Mutual Info/Frac of variance (R^2 ceil)
  4. [ ] Nonlinear Facilitation model
  5. [ ] Gain control stateful model
  6. [ ] Bayesian nonparametric T-test instead of randttest
  7. [ ] Make a script for inter-module overall contribution/comparion
	 ...
	 %     Plot the constants
  - Low rank models paper by Ahrens and Sahani

* Meeting with SVD 2014-03-03
  ...
  - NARF stripped down version
    1) Built-in fitter and default model, single simple interface takes matrices
    2) Be able to add other streams like "pupil diameter" and work on possibly interleaved data "blocks"
    3) First fits one channel, then adds another as needed until no more significant improvements are found
...
* BUG SVD ABOUT
  ...
  - Is there a plan to ever take recordings from the Cochlear Nucleus, Superior Olivary, Inferior Colliculus, Medial Geniculate Nucleus, (A1), or Non-A1 cortex?
  - Is there XYZ positional data so I could start connecting best-fit parameters to cortical location?

* ASSUMPTION TO CHECK
  If the noise distribution of a PSTH does indeed approach a gaussian, (regardless of the poisson distribution or bernoulli per trial), how many trials are realistically needed before the distribution becomes really gaussian?
  The "improvement" from multiplicative nonlinearities is probably due to our stupid way of fitting with MSE weighting a '2-spike error' four times more than a 1 spike error. This desperately needs to be fixed.

* WHILE DISCUSSING WITH DAN
  I have been hesitant to use bayesian analysis (like the log likelihood) on my data because I didn't know how to convert a predicted spike rate P into an actual probability of a spike. But now I realize that is actually pretty damn simple; 1/P = lambda, the poisson process mean rate. From there it's a very simple step to compute the ISI of the spike train, and using the inverse of CDF(lambda), to figure out how likely that spike train is. 
  I should run this for many of the models, and try to work out the "neighboring likelihoods" of each of the parameters, so I can see how confident I am of each parameter.
  
* HOW TO COMPARE MODELS IN A BAYESIAN WAY:
  I think the new perspective on fitting a neuron is to do this:
  - Fit a variety of models to a neural data set using our non-bayesian approach. 
  - These fits become the "prior" of the neurons.
  - Start with 10 model hypotheses about the compressor (EXP, ZTHRESH, SIGLOG, ETC) (hyperprior: equal prob of models)
  - Insert data. Refit model using MCMC to get posteriors. Throw out outliers. Repeat MCMC again.  
    - Note that essentially this simultaneously searches a space with a number of dimensions equal to the sum total of # of params of all models, plus one dim to INDEX which model we are studying.
    - This means models with more params be sampled as less believable (which kind of already incorporates regularization, which may be irritating)
  - We can immediately say which model is the winner for a particular neuron. 
  - Add another nonlinearity. Wash, rinse, repeat. 
  - Interesting! We face the same problems we face now in a Bayesian system: regularization due to Bayes can cloud conclusions. I think I prefer ratios of likelihoods so I don't get confused by priors (Just the data, please).

* Clojure MCMC
  Ideally, we could represent a parameter space, (and it's associated prior/posterior probability distrubution) using something that is GRID-LIKE, SMOOTH, and SPARSE. 
  Priors and hyperpriors are basically the same; terms express the relationship of dependencies.
  Pseudopriors are basically a hack to use the posterior dist to make the sampling occur in the vicinity of believable values. The prior is still used in the end; but we need a hacky way of bootstrapping the sampling distribution when it is poorly known.
    
* Latest from SVD
  1. Make a log-likelihood fitter that varies each parameter and tests log-likelihood ratios. 
  ...
  2. Rebuild Pole-Zero module to work on multiple channels, with multiple PZ filters per channel. 
     Have a function that can sample IIR filter. 
  3. Make a script that runs on 2 models over a whole population and compares 
...
* Work Queue
  1. [ ] Narf Analysis needs refactoring. Query DB based on selections, filter by >r_floor and by fairness/presence across all models, then display in bottom window. That restricted data can then be used directly for plotting rather than doing a new lookup every time, so things are faster.
  1. [ ] Look just at histograms for >10^-2 latencies (less than that is probably a fitting bug)
  2. [ ] Plot Pole1 vs Pole2 for p1z0x2 to see if we have fast/slow pole cluster
  3. [ ] Module: Take the FFT of a spike train, then filter out frq components above some threshold to make a smoothed RESPAVG.
  4. [ ] Verify that the fittable gammatones use Hilbert Envelope correctly
  ...
  7. [ ] Metric: Log-likelihood
  8. [ ] Metric: NMSE in DB
  9. [ ] Metric: Classifer model (spike = 1, nospike = -1, error function = (1-r_hat*r)^2
  10. [ ] Fitter: Not using an input channel to each module on some steps. (NAN out a chan?)
  11. [ ] Fitter: skipping some model parameters each step.
  12. [ ] END PLOT: Heatmap + raster of resp
  13. [ ] Plot R/sigma^2 vs spike isolation level 
  14. [ ] Plot noise ceiling vs spike isolation
  15. [ ] Plot # params vs avg performance
  16. [ ] Try different initial conditions for NIM models?
  17. [ ] Compare a slow pole vs depression vs other depression ideas
  18. [ ] Optimize the Pole-Zero models so that they run faster
  19. [ ] Ideas for depression or gain control?
  20. [ ] Show that "delayed inhibition" effect is an aliasing of something else.

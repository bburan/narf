* Work Queue
  1. Refit/Queue things at 200Hz. 
  2. Double check scatter/bar plot accuracy
  4. Add normalization by default just before the nonlinearities to make fitting faster
  5. Multiple p1z0 summed together (Start with 2)
     1) [ ] Per-channel input nonlinearities
     2) [ ] Try p2z0 or the best in general 
  6. A function to "store" current fittables, and then "restore" fittables, so I can quickly fit just the most recent module in a keyword
  ...
  10. Nonlinearity: F(x) = alpha * log ( 1 + exp(beta*(x-theta))), where scale is alpha, shape is beta, and offset is theta. 
  ...
  13. SVD Request: Study overfitting (analysis 19, batch 246). 

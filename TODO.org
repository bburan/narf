...
* Friday
  ...
  - [X] Inspect Fitters
  - [ ] Inspect NLxCP variations
  - [ ] Queue the depression survey
  - [ ] A script to create a new DB cache with all metrics
  - [ ] Runscript on 240 to create avg fit time cache
...
* Engineering
  ...
  5. [ ] NARF Architecture Improvements
  ...
  7. [-] Default per-paramset, per-channel heatmap graph fns:
	 - [ ] Loadstimfrompbaphy
  ...
  9. [ ] Safety check so that these modules do not allow unifiers/splitters
  ...
  12. [ ] Profile and look for optimizations:

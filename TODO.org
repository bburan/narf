* BEFORE TRAINING STARTS AGAIN
  ...
  3. [ ] Generic nonlinearity should accomodate scatter plots of multi-input things
  4. [ ] Plot both test and training data for smooth scatter plots.
  5. [ ] Make the fits run faster by removing then re-adding the 'test_set' data after fitting process
  6. [ ] Put a line in fit_single_model that pulls the latest GIT code before fitting?
  7. [ ] When you click save, it should bring up a 'trainerstring' name so you can customize how the model is saved.
  ...
  8. [ ] Plot_model() should scan model to look for plotabble fns
  ...
  9. [ ] FIR module should have multiple sets of COEFS so it can do per-file filtering
  10. [ ] NPNL module should have multiple sets of NLs allowed to float on each


* KEYWORDS
  1. [ ] Init FIR to rcorr
  2. [ ] Init FIR to zero
  3. [ ] Init FIR to some prior
...
* Architectual Improvements
  ...
  6. [ ] Bayesian Performance Metrics
...
* SMALL CORRECTIONS AUDIT LIST
  1. [ ] Is this line fine with the closures? m.plot_fns{1}.fn = @(stack, xxx) do_plot_channel_vs_time(stack, xxx, m.time, m.output);
  2. [ ] IRRITATION: Why doesn't it show the model save filename so I can see which file I just loaded if I forgot?
  3. [ ] IRRITATION: Why doesn't every plotted signal have a legend?
  4. [ ] IRRITATION: Why don't the X (or at least the Y) axes have scales?
  ...
  7. [ ] Replace my nargin checks with "if ~exist('BLAH','var'),"
  8. [ ] Replace all the 'true' and 'false' arguments with textual flags and varargin that are more descriptive
  9. [ ] Make sure that fitters return specific codes indicating how they terminated
  10. [ ] Pull out repeated code blocks in the fitters (because they are all pretty much the same damn thing over and over again)
  11. [ ] Repair NarfModelpane
  ...
  18. [ ] Add a precompressor keyword which concats log compressor as an extra channel
  19. [ ] Make Concat Second Order Terms work for any higher order nchoosek type stuff
  20. [ ] Provide functions to cover an input space logarithmically with filters
  ...
  28. [ ] sf=sf{1}; should be eliminated IN EVERY SINGLE FILE!
  29. [ ] Grep for TODO's, FIXME's, etc in existing files and add them to this list
...
* DISCARDED WORK
  ...
  8. [ ] DB Bug Catcher which verifies that every model file in /auto/data/code is in the DB, and correct

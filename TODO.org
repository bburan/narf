...
** Data Structures
   ...
   - NarfBatches
     | batch         | varchar(255) |   |   |   |   | More generic names                                   |
     | answer        | text         |   |   |   |   | Our current answer.                                  |
     | modeltree     | text         |   |   |   |   | A model keyword tree; all branches are compared      |
     | batch         | int(11)      |   |   |   |   | Associated with a particular batch                   |
     | XXX0          | text         |   |   |   |   | cellid, est_set, val_set, filecodes                  |
...
* Actions
  1. [ ] Make the narf-workers path different from my dev path
  2. [ ] Fix write permissions for images, models
  3. [ ] Try modifying the adding SQRT to of the RESPAVG 
  4. [ ] A better queuing script
	 ...
	 - [ ] Force git clean?
	 - [ ] Check for dead/jobs
	 - [ ] Rerun analysis with current?
	 - [ ] Do DB contents and Filesystem still sync up?
	 - [ ] Does a check. If any models don't sync up, they will be enqueued to the jobs system.
  5. [ ] Tool to start comparing models:
  ...
  7. [ ] Default per-paramset, per-channel heatmap graph fns:
	 ...
	 - [ ] FIR filter
	 - [ ] Nonlinearity
  8. [ ] Scatter plots with distributions
  9. [ ] Add functionality
  ...
  11. [ ] Clean up saved_analysis, saved_models, saved_images
  12. [ ] Repair Fitters 
  ...
  13. [ ] Add new functionality to the do_scatter_plot method
	  - [ ] Instead of plotting a scatter plot as points, use a fine-grid HEAT MAP
		Use grayish/blackish 
  14. [ ] jackknifing needs to work with other performance metrics?
  15. [ ] Profile the time spent during boosting and look for optimizations:
  ...
  16. [ ] Possible features to extract (And what what is needed to detect them)
  ...
  17. [ ] What if we use RESPAVG to compute the depression state, and fit the depression amounts?
  18. [ ] NPNL with autocorrelation of last few ms
  19. [ ] What are the covariances of the FIR coefs? (RCORR)

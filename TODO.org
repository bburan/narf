* Work Queue
  1. Try to get "channel weighting" module working
  1. For Vocalization Data:
     + 12CH gammatone
     + channel smoothing with 6 gaussians
     + Depct1
     + NIM with polezero and/or FIR
  5. Add normalization by default just before the nonlinearities to make fitting faster
  6. Metric: Log-likelihood
  ...
  8. Fitter: Not using an input channel to each module on some steps. (NAN out a chan?)
  9. Fitter: skipping some model parameters each step.
  10. SVD Request: Study overfitting (analysis 19, batch 246). 
  ...
  11. Classifer model (spike = 1, nospike = -1, error function = (1-r_hat*r)^2

* Done
  1. Try adding a clojure app to the java classpath and calling a clj function from matlab.

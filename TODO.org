* HIGH LEVEL OBJECTIVES:
  1. [ ] Find a fitter such that DEPFREE always matches or exceeds FIRNO's performance
  2. [ ] Add more metrics so we can answer subtle questions better
  3. [ ] Add a two-model comparison function which scatter-plots many matrices, does a histogram of collapsed scatter plot along diagonal, and does randomized paired T-test. Maybe log-likelihood ratio too. 
  4. [ ] Grant: (Look at NRSA grant from NIH, due in December, called the Kirchstein award, F-32. I would be a good applicant, and could probably modify SVD's grant and list my own publication.  Also, a hearing health foundation grant might also help. ).  Work through the grant example stephen sent me
...
* Metric-taking Ideas being developed in update_metrics();
  ...
  10. [ ] metric_nmse: "Normalized mean squared error" (R_pred - R)^2 / (R^2)
  11. [ ] metric_binned_resp_corr: RESP's variance with itself at this bin size.
  12. [ ] metric that assumes a gaussian noise interfering with prediction and RESPAVG, and gives back an "improbability" score that is to be minimized
  13. [ ] metric_corr_homoskedastic: Ensure homodskedasticity before taking correlation by reweighting
  14. [ ] metric_corr_trim_outliers: Remove outliers from the correlation
  15. [ ] metric_mse_trim_outliers.m: Takes the MSE without outliers
  16. [ ] metric_vr.m: Distance metric between predicted and actual spike trains
  17. [ ] metric_rectified: Rectified correlation approximation
  18. [ ] metric_mutual_info.m: Assume a gaussian noise model or something?
...
* Next Steps:
  1. Presentation of Maximially Informative Dimensions
  2. Make split modules visible/editable from the GUI
  3. Read about the Grant and write a proposal. Stephen suggests I write about:
  ...
  7. Modelstring bug.
  8. Mixfit variations explored more.
  9. Queue up a few different smooth_respavg kernels at 200Hz  
  10. A plot of # of free parameters (put in database)
  11. Profile code and look for optimizations that would speed up each iteration
  12. Why does SENL drop to keyboard?
  13. Heatmaps: loadstimfrombaphy, FIR, nonlinearity
  14. Metrics:
  ...
  15. Updated DB Insertion and Great Name Replacing proposal
  ...
  19. Write a generalized hook-in for constraints (  - Zeroing or Naning out values to express a constraint)

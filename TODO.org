* Iterative boosting
  - Quick & dirty boost algorithm that fits each module separately. 
...
* NEXT STEPS:
  - [X] Termination condition not absolute, but relative to initial starting point
  - [X] Find out why err10, err15 not working (Interference from some plotting code SVD inserted while debugging)
  - [ ] Boosting variant: Cheat and stop whenever the validation set error goes up.
  - [ ] Boosting variant: Fit each module separately
  - [ ] Boosting variant: terminate when last N times the 10% held out data error has gone up 
  - [ ] pfs (post-fit-shrink based on effect)
  - [ ] Put sparsity metric stuff into the DB so NPFNL vs NPNL can be compared on that axis

* Engineering:
  1. [ ] TALK WITH STEPHEN, AND WHEN THE QUEUE IS EMPTY, DO THE VALIDATION/TEST  and EST/TRAINING  set naming replacements!
  2. [-] Default per-paramset, per-channel heatmap graph fns:
	 - [ ] Loadstimfrompbaphy
	 - [X] Nonrmalize channels
	 - [ ] FIR filter
	 - [ ] Nonlinearity
  3. [ ] 'scripts' directory which can let you apply selected models to a given script for plotting, comparison, or matrix-building
  4. [ ] Safety check so that these modules do not allow unifiers/splitters
         - Correlation, MSE, bayesian, loader, etc
	 - Split normalization or not?
  5. [ ] During insert into DB, take a bunch of metrics
  ...
  7. [ ] Profile and look for optimizations:
...
* Experiments:
  ...
  3. [ ] Can a really simple model like this do very well:
  ...
  8. [ ] Sparsifying based on relevancy of coefficients	 
...
* THE GREAT NAME REPLACING PROPOSAL
  ...
  3. [ ] NarfResults -> NarfModels
  4. [ ] XXX -> ??
  5. [ ] MODULES (What would this become? )
  6. [ ] META -> (Suggestion: Should this be MODELINFO, instead of just 'metadata'?)
  7. [ ] STACK -> (Suggestion: Should this be MODULES, MDLS, etc?)
  8. [ ] Make a list of every function used purely for side effects, and rename it with a ! at the end
  9. [ ] WHATEVER IS GOING INTO XXX{1} should be given to fit_single_model as well! When I'm not using BAPHY it should still be able to work.

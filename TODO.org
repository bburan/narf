* Work Queue
  1. Keep hammering on Vocalization Data w/12CH gammatone, smoothing, dep, NIM
  2. Frankly, I think NORMALIZATION AT EVERY SINGLE STEP, except for the last step which has shift and scale, would be an excellent idea.
  4. Metric: Log-likelihood
  5. Fitter: Boosting with param size scaling (Take variance between current point and slight deviation. Take a VERY SMALL deviation (10^-6, at first. If that's too small, try factors of 10)
  6. Fitter: Not using an input channel to each module on some steps. (NAN out a chan?)
  7. Fitter: skipping some model parameters each step.
  8. SVD Request: Study overfitting (analysis 19, batch 246). 
  ...
  9. Classifer model (spike = 1, nospike = -1, error function = (1-r_hat*r)^2

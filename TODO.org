...
* Corrections discovered
  - Sigcauchy completely wrong as compressor?
  - Sigumber mostly wrong
     
* Friday
  - [X] Inspect parametric compressors (See report)
  - [X] Inspect Rerun normalization with "force positive" -> (Only helps CP3Free)
  - [X] Inspect parametric NLs -> winner is SIGLOG 
  - [X] Inspect Fitters
  - [ ] Scripty stuff
    - [ ] Apply a function 
    - [ ] A script to create a new DB cache with all metrics, fit times
    - [ ] Runscript on 240 to create avg fit time plot
  - [ ] Inspect NLxCP variations, and queue depression survey
  - [ ] Inspect depression survey when complete
  - [ ] Write a smarter booster with elitist stepping
  - [ ] Figure out why there is no test or train correlation with  fit_single_model(240,'por026c-c1',{'env100','sigumber','firno','initrc','nonl','mse','boost',},{'por026c10_p_SPN',}, {'por026c09_p_SPN',}, {})
  - [ ] Bugfix: when using init0, siglog/nonl doesn't work

* Engineering
  1. [ ] Bugfix: why does IVAR get called? (traced it to strsep by SVD...I guess it didn't do what I thought it did!)
  1. [ ] Bugfixes
	- Modelstring bug
  ...
  2. [ ] Extract the ending conditions of many fitters and try to improve initial conditions
  ...
  5. [ ] Optimization: If a keyword string already exists, load that rather than refit. 
  6. [ ] NARF Architecture Improvements
  ...
  8. [-] Default per-paramset, per-channel heatmap graph fns:
	 - [ ] Loadstimfrompbaphy

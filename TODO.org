* SUMMARY OF CHANGES:
  ...
  9) [ ] Substantial work in utils/
	 1. [ ] delete_module_gui
	 2. [ ] delete_all_module_guis
	 3. [ ] verify_model_polarity
	 4. [ ] get_baphy_plot_controls
	 5. [ ] plot_module_summary
	 6. [ ] custom_demo?
	 7. [ ] find_modules
	 8. [ ] 
  10) [ ] Substantial work in narf_modelpane
	  Pretty much every time you use STACK{...} you need to check!
	  This is a pain, but does it occur anywhere else?
  11) [ ] Whatever calls plot hooks in narf_modelpane now

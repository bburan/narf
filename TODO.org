* ABCD
  Model additions that may help improve ABCD:
  1. [X] Y offset.  (Not needed now that there is a constant x offset built into B)
  2. [X] Per-channel latencies. 
  3. [ ] Per-channel input nonlinearities. 
  4. [ ] An output nonlinearity. However, I think this should be added and fit AFTER the ABCD. It's hard enough to fit ABCD already. 
  5. [ ] Comparing things at 200Hz not 100Hz....no extra parameters!
  6. [ ] 
...
** Naive Bayes
  The more I think about it, just using naked probabilities (MAP) seems more and more attractive.
  For example, when we calculate MSE, the stupid "prestim-silence" distorts the histograms because so many equivalent values come in.
  These would be naturally weighted less if we were using the bayesian approach to marginalize by the expected data. 
...
* WORK QUEUE
  - SVD: Overfitting (analysis 19, batch 246). 
...
* Ideas:
  ...
  - Could parameterizations be explicitly built into a matrix form?
...
* Next Steps:
  ...
  5. Read Dan Butz NIM paper and package

* ROADMAP
  1. [X] Input-spanning depression bank (50,200,500ms recovery values, and 0.5, 1.5, 2.5 depression rates)
  2. [ ] Finish the big analysis batch and choose values
  3. [ ] Start using BAPHY for caching analysis results
  4. [ ] Cleaning, bugfixes, and user interface changes that have accumulated
  5. [ ] Address question: Does variation in neural fuction as in A1 follow a continuum, or are there visible clusters?
...
* WORKFLOW CHANGES
  ...
  - [ ] Need a way of marking a particular 'block' so that particular 'plots' can be extracted despite positional differences
...
* WORK
** TODAY
   1. [X] Plot results so far, help brian get started on them
   2. [X] Fix cholskey decomp error and get npfnl started again (Fixed by finding closest positive definite matrix)
   3. [X] Try some test jobs using job distribution code (FAILURE)
   4. [ ] Fix EM conditioning error and get gmm4 started again (Not sure how to fix!)
   5. [ ] Partition code into fitting and plotting sections

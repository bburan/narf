...
** Growing at 1.5 is clearly too much
...
** Two dependent parameters
   Imagine one parameters that affect each other, one with a large magnitude and the other with a small magnitude
   The stepsize will have to be VERY small before the smaller one will be modified. 
...
* TODAYS WORK
  ...
  - [ ] Add a fit_scaat algorithm which should be good for 'cleanup'?
  - [ ] MSE NORMALIZATION

* ROADMAP
  1. [ ] Input-spanning depression bank (50,200,500ms recovery values, and 0.5, 1.5, 2.5 depression rates)
  2. [ ] Start using BAPHY for caching analysis results
  3. [ ] Cleaning, bugfixes, and user interface changes that have accumulated

* WORK
** TODAY
   1. [ ] Plot results so far
   1. [ ] Fix cholskey decomp error and get npfnl started again
   2. [ ] Fix EM conditioning error and get gmm4 started again
   3. [ ] Try some test jobs using job distribution code

** WITH BRIAN
   1. [ ] Estimate 'best' MSE sparseness penalty
   2. [ ] Look for 'weirdness', see if sparseness is 'working'
   3. [ ] 

** MINOR ADDITIONS
  1. [ ] If empty test set is given for a cellid, what should we do? Hold 1 out cross validation? 
  2. [ ] Add a keyword with a log compressor as an extra channel
  3. [ ] More concise summary of a cellid's models. HTML data table output?
  4. [ ] Plot both test and training data for smooth scatter plots.
  5. [ ] Standardize colors for plot lines. 
  6. [ ] A histogram heat map of model performance for each cell so you can see distribution of model performance
  7. [ ] A way to compare nonlinearities for multiple data sets.  
  8. [ ] Can I hide windows when plotting on them so they don't pop up and ruin my day when coding?
  9. [ ] FN: 'set_module_field' (finds module, sets field, so you can mess with things later in scripts)

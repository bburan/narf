...
* Why is per-file boosting so crappy?
  - Is correlation being calculated properly?
  - Are we splitting and unifying on the files trained upon?
  - Why is the training R^2 not improving? We should at least overfit better!
  - Does MSES have an effect?
  - Init0?
  - Is the boosting fitter quitting to soon? (Try lsq)
  - Normalization is done across files and could affect things subtly
  
* My options
  - Figure out how to accomodate splits before the boostperfile
  - Cache
  - Make 

* ADD
  - Safety check so that these modules do not allow unifiers/splitters
    - correlation
    - MSE
    - bayesian
    - loader
...
* Brainstorm of improvements to log likelihood metric:
  - [ ] Try decimate, or resample (@(x,n) abs(decimate(x,n)),  @(x,n) abs(resample(x,1,n))
  - [ ] Try BIC instead of log likelihood
  - [ ] Try using the gamma distribution or inverse gaussian
  - [ ] Try truncating in different ways, try shifting or no shifting 

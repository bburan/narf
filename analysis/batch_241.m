...
% 2. Uses the NPNLX module, which lets the NPNL float per-file across the
...
mm{5} = module_groups('boost');  % The 3 best fitters: boost, 'sb', 'fminlsq'
mm{6} = module_groups('mse', 'mses1', 'mses2', 'mses3', 'mses4', 'mses5'); 

modulekeys = module_block_combos(mm); % Combinatorial fun!

for ii = 1:length(cells)
    for jj = 1:length(modulekeys)
        fprintf('Fitting model [%d/%d]\n', jj, length(modulekeys)); 
        
        % Uncomment next line to fit models immediately on this machine

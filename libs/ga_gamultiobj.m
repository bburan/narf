function [x,fval,exitFlag,output,population,scores] = ga_gamultiobj(fun,nvars,Aineq,bineq,Aeq,beq,lb,ub,options)
...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:08:17 $
...
if ~isempty(errmsg)
    error('globaloptim:gamultiobj:numberOfInputs',[errmsg,' GAMULTIOBJ requires at least 1 input argument.']);
end
...
if nargin == 1
    if isa(fun,'struct')
        ...
        error('globaloptim:gamultiobj:invalidStructInput','The input should be a structure with valid fields or provide at least two arguments to GAMULTIOBJ.' );
    end
end
...
if isempty(FitnessFcn) ||  ~(isa(FitnessFcn,'inline') || isa(FitnessFcn,'function_handle'))
    error('globaloptim:gamultiobj:needFunctionHandle','Fitness function must be a function handle.');
end
...
valid =  isnumeric(nvars) && isscalar(nvars)&& (nvars > 0) ...
    && (nvars == floor(nvars));
if ~valid
    error('globaloptim:gamultiobj:notValidNvars','Number of variables (NVARS) must be a positive integer.');
end
user_options = options;
% Use default options if empty
if ~isempty(options) && ~isa(options,'struct')
        error('globaloptim:gamultiobj:optionsNotAStruct','Ninth input argument must be a valid structure created with GAOPTIMSET.');
...
try
    ...
    if ~isequal('double', dataType)
        error('globaloptim:gamultiobj:dataType', ...
...
[x,fval,exitFlag,output,population,scores,FitnessFcn,nvars,Aineq,bineq,Aeq,beq,lb,ub, ...
    NonconFcn,options] = ga_gacommon(nvars,fun,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options,user_options);

if exitFlag < 0 % Infeasibility
    return;
end

% Call appropriate multi objective optimization solver
[x,fval,exitFlag,output,population,scores] = ga_gamultiobjsolve(FitnessFcn,nvars, ...

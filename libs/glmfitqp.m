function results = glmfitqp(y,X,qf,opts)
    ...
    if ~strcmp(opts.family,'normid')
        %Refine
        w = irls(y,X,w,qf,opts.baseline,opts.family,opts.familyextra,opts.algo,opts.Display,opts.weights);
    end
    
    %{
    This doesn't work yet
    if opts.getLOO
        %Get Leave-one-out cross-validation score
        [~,~,Hd,r] = evalGlmLikelihood(y,X,w,opts.baseline,opts.family,opts.familyextra);
        
        HiX = (results.H\X')';
        XHiX = sum(X.*HiX,2);
        %term1 = bsxfun(@times,HiX,(X*w).*Hd./(1+Hd.*XHiX));
        dw    = bsxfun(@times,HiX,r);
        term2 = bsxfun(@times,HiX.*X.*bsxfun(@plus,dw,w'),Hd./(1+Hd.*XHiX));
        %%
        dwtotal = term2 + dw;% - 0*term1;
        %median(diag(corr(dwtotal,bsxfun(@minus,W,mean(W)))))
        %%
        %eta = X*(w+dwtotal);
        
        llloo = evalGlmLikelihood(y,sum(X.*bsxfun(@plus,w',dwtotal),2),1,opts.baseline,opts.family,opts.familyextra);
        results.loologlikelihood = -llloo;
    end
    %}
...
function [w] = irls(y,X,w,qf,b,family,familyextra,algo,Display,weights)
    opts.Display = Display;
    if strcmp(algo,'medium')
        opts.Display = 'off';
        opts.LargeScale = 'off';
        opts.GradObj = 'on';
        w = fminunc(@(w) evalLplusp(y,X,w,b,qf,family,familyextra,weights),double(w),opts);
    elseif strcmp(algo,'large')
        opts.LargeScale = 'on';
        opts.GradObj = 'on';
        opts.Hessian = 'On';
        w = fminunc(@(w) evalLplusp(y,X,w,b,qf,family,familyextra,weights),double(w),opts);
    elseif strcmp(algo,'lbfgs')
        opts.Method = 'lbfgs';
        w = minFunc(@(w) evalLplusp(y,X,w,b,qf,family,familyextra,weights),double(w),opts);
...
function [f,g,H] = evalLplusp(y,X,w,b,qf,family,familyextra,weights)
    if nargout > 2

...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:09:16 $
...
if ~isempty(options.InitialScores) && numObj ~= size(options.InitialScores,2)
    error('globaloptim:gamultiObjMakeState:initScoreSize','size(InitialScore,2) should be equal to number of objectives');
end
...
if totalPopulation ~= initScoreProvided
    ...
    if strcmpi(options.Vectorized, 'off')
        ...
        if size(Score,1) ~= individualsToEvaluate
            ...
            error('globaloptim:gamultiObjMakeState:fitnessVectorizedCheck',msg);
        end
        state.Score(initScoreProvided+1:end,:) =  Score;
    end
...
[state.Population,state.Score,state.Rank,state.Distance] = ...
    ga_rankAndDistance(state.Population,state.Score,options,popSize);
...
if (length(parents) ~= nParents) || min(parents) <= 0 || max(parents) > popSize
   error('globaloptim:gamultiobjMakeState:selectionFcnError','SelectionFcn must return positive array of length %d with values in the range [1 %d].',nParents,popSize);
end
...
for pop = 1:length(totalPop)
    ...
    ga_distanceAndSpread(state.Distance(subpopIndex,:),state.Rank(subpopIndex,:), ...

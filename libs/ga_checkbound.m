function [lb,ub,msg,exitflag] = ga_checkbound(lbin,ubin,nvars)
...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:08:59 $
...
if lenlb > nvars
       warning('globaloptim:checkbound:lengthOfLowerBound','Length of lower bounds is > length(x); ignoring extra bounds.');
       lb = lb(1:nvars);   
    lenlb = nvars;
elseif lenlb < nvars
...
if lenub > nvars
       warning('globaloptim:checkbound:lengthOfUpperBound','Length of upper bounds is > length(x); ignoring extra bounds.');
       ub = ub(1:nvars);
    lenub = nvars;
elseif lenub < nvars
...
if any(eq(ub, -inf)) 
   error('globaloptim:CHECKBOUND:infUpperBound','-Inf detected in upper bound: upper bounds must be > -Inf.');
elseif any(eq(lb,inf))
   error('globaloptim:CHECKBOUND:infLowerBound','+Inf detected in lower bound: lower bounds must be < Inf.');
end

...
if ~exist('rawids','var'),
   ...
   disp('Repreocessing pype data and generating new sCellFile entries.');
   sql=['SELECT gDataRaw.*,sCellFile.id as cellfileid,',...
        ' sCellFile.stimfilefmt',...
...
for rawidx=1:length(rawdata),
   if ~GENERALUPDATE & ~isempty(rawdata(rawidx).cellfileid),
      ...
      if ~findstr(rawdata(rawidx).info,'CELL'),
         ...
         elseif strcmp(z.h.stim(1:2),'be') | strcmp(z.h.stim(1:2),'sy'),
            ...
            stimfile={[stimbase((ii+1):end),'.',z.h.index0,'.pix.imsm']};
            stimfmtcode={0};
            stimfilefmt={'pixel'};
            ...
            stimbase=[stimbase((ii+1):end),'.',z.h.index0,'.'];
...
[cellfiledata,cellids,cellfileids]=dbgetscellfile(rawids,fmt);

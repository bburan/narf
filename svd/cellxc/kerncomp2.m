...
for fidx=1:filecount,
   ...
   %bresp=bresp-repmat(brmean,[blen 1 1]);
   ...
   eigstim=bstim * ua(:,1:spacelim);
   
   eigmpatches=(mpatches'-repmat(bsmean,[attuse,1]))*ua(:,1:spacelim);
   eigfpatches=(fpatches'-repmat(bsmean,[patchcount,1]))*ua(:,1:spacelim);
   
   bincount=10;
   rbincount=zeros(bincount,respcount,attcount);
   rbinned=zeros(bincount,respcount,attcount,spacelim);
   rbinnedstd=zeros(bincount,respcount,attcount,spacelim);
   mpbinned=zeros(attuse,spacelim);
   sbin=zeros(bincount,spacelim);
   
   % cycle through each stimulus eigenvector
   for eigidx=1:spacelim ,
      fprintf('\nbinning r: eigidx=%d ',eigidx);
      ...
      for bb=1:bincount
         ...
         if eigidx==1,
            rbincount(bb,:,:)=sum(~isnan(bresp(sbbidx,:,:)),1);
         end
         
         % determine mean and std response for stimulus bin bb
         rbinned(bb,:,:,eigidx)=nanmean(bresp(sbbidx,:,:));
         for attidx=1:attcount
            rbinnedstd(bb,:,attidx,eigidx)=...
                nanstd(bresp(sbbidx,:,attidx)) ./ sqrt(rbincount(bb,:,attidx));
         end
   ...
   disp('Calculating T2 and significance...');
   DOPAIRS=1;
   if DOPAIRS,
   ...
   r2=reshape(rbinned(:,:,2:end,:),bincount,respcount,attuse-1,...
              noisesets,spacelim);
   r2std=reshape(rbinnedstd(:,:,2:end,:),bincount,respcount,attuse-1,...
              noisesets,spacelim);
   for respidx=1:respcount,
      for pidx=1:attdocount,
         ...
         a2=attpairs(pidx,2);
         for eigidx=1:spacelim,
            x1=permute(r2(:,respidx,a1,:,1:eigidx),[1 5 2 3 4]);
            x1=reshape(x1,bincount*eigidx,noisesets);
            x2=permute(r2(:,respidx,a2,:,1:eigidx),[1 5 2 3 4]);
            x2=reshape(x2,bincount*eigidx,noisesets);
            
            x1std=permute(r2std(:,respidx,a1,:,1:eigidx),[1 5 2 3 4]);
            x1std=reshape(x1std,bincount*eigidx,noisesets);
            x2std=permute(r2std(:,respidx,a2,:,1:eigidx),[1 5 2 3 4]);
            x2std=reshape(x2std,bincount*eigidx,noisesets);
            xstd=(x1std+x2std)./2;
            xstd(find(xstd==0))=1;
...
clear bstim bresp x1 x2 r2 r2std

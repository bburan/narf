% kerncomp5.m : compare kernels from different attentional states to
...
disp('kerncomp5.m : dealing with dc/global scaling issues');
...
%keyboard
%
% regularization code ripped out of normalizereg.m
%
lambdaomag=6;
nfactors=20;
sD=diag(sa);
...
for sfsidx=1:nfactors,
   ...
   xct(sfsidx)=xcov(r0(agoodidx),ract(agoodidx),0,'coeff');
end

ddmax=min(find(xct==max(xct)));
Hdamp=sD(find(sD>0))./(sDnz+lambda(ddmax)./sDnz);
if sum(Hdamp)==0,
   disp('lame situation: Hdamp = all zeros.');
end
Hdamp=repmat(Hdamp,[1 attcount]);

fprintf('maximum DAMPFACTOR=%.1f (id=%d)\n',lambda(ddmax),ddmax);

if 0
   % old damping. seems to be a bad way of doing it because noise
   % distributes weirdly in eigenspace
...
if ~DODAMPING,
   ...
   xctb(:,:,1)=rconv2(xctb(:,:,1),[1/3 1/3 1/3]');
   
   %emax=min(find(xct(:,1)==max(xct(:,1))));
   ...
   for bootidx=1:bootcount,
      if sum(~isnan(xctb(:,bootidx)))>0,
         bemax(bootidx)=min(find(...
            (xctb(:,bootidx,1)-min(xctb(:,bootidx,1)))>= ...
            (max(xctb(:,bootidx,1))-min(xctb(:,bootidx,1))).*0.95));
         %bemax(bootidx)=min(find(...
         %   xctb(:,bootidx,1)==max(xctb(:,bootidx,1)) ));
      end
   end
   ...
   if predlims<2,
      fprintf('predlims was %d; forcing up to 2\n',predlims);
      predlims=2;
   end
...
for noiseidx=1:params.noisecount+1,
   ...
   for p1=2:attcount-1,
      for p2=(p1+1):attcount,
         ...
         xcp(noiseidx,3,pidx,xcidx)=xcov(rglobalatt(aattidx),...
                                         ractnodc(aattidx),0,'coeff');
         xcp(noiseidx,4,pidx,xcidx)=xcov(rfullatt(aattidx),...
                                         ractnoglob(aattidx),0,'coeff');
      end
   end
   ...
   for p1=1:attcount,
      ...
      xccross(noiseidx,3,p1,xcidx)=...
          sqrt(mean((rglobalatt(aattidx)-ractnodc(aattidx)).^2))./std(ractnodc(aattidx));
      xccross(noiseidx,4,p1,xcidx)=...
          sqrt(mean((rfullatt(aattidx)-ractnoglob(aattidx)).^2))./std(ractnoglob(aattidx));
   end

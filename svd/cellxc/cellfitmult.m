...
testcount=2;
fprintf('cellfit.m .... multiple expl sets (%d)\n',testcount);
...
xc=zeros(sfscount,sigcount,latcount,nlcount,attcount,testcount);
...
for attidx=1:attcount,
   ...
   for testidx=1:testcount,
      ...
      teststart=round((testidx-1)*size(resp,1)/testcount)+1;
      teststop=round(testidx*size(resp,1)/testcount);
      if testidx>1,
         teststart=teststart-min([teststart-1 tbincount]);
      end
      
      fprintf('Test set 1: %d-%d\n',teststart,teststop);
      
      tr=resp(teststart:teststop,:,attidx);
      tr(1:tbincount,:)=nan;
      ...
      for r=1:respcount,
         if meansub,
            ...
            % set correctly
            tstim=stim(:,teststart:teststop) - ...
                  repmat(mSall(:,r,attidx),[1 teststop-teststart+1]);
         else
            tstim=stim(:,teststart:teststop);
         end
         for sfsidx=1:sfscount,
               ...
               for nlidx=1:nlcount,
                  ...
                  for ridx=1:latcount,
                     ...
                     if std(tmod_psth(tgoodidx{ridx},ridx))>0,
                        xc(sfsidx,sigidx,ridx,nlidx,attidx,testidx)=...
            ...
            if not(isempty(BATQUEUEID)),
               % record latest (r,attidx) predicted
   ...
   for nlidx=1:nlcount,
      for latidx=1:latcount,
         ...
         % figure out max xc
         %txc=mean(xc(:,:,latidx,nlidx,attidx,:),6);
         txc=min(xc(:,:,latidx,nlidx,attidx,:),[],6);

...
% ALLNL flag determinees whether output nls get fit to each
% (sfsidx,sigidx) pair (ALLNL=1), or if the optimal regularization
% is selected before fitting the att-localized dcg fits (ALLNL=0);
ALLNL=1;
if ALLNL,
...
for sfsidx=1:sfscount,
   ...
   for respidx=1:respcount,
      ...
      for sigidx=1:sigcount,
         ...
         respforfit=respsave;
         if ALLNL,
            tstrf=cat(1,repmat(tstrf1(:,1),...
                               nlcount-length(localnls),attcount-1),...
            ...
            for nlidx=1:nlcount,
               ...
               for fitidx=1:attcount-1,
                  ...
                  nlparmset{sfsidx,sigidx,nlidx,fitidx}=...
                      tstrf(nlidx,fitidx).nlparms;
                  nlnameset{sfsidx,sigidx,nlidx,fitidx}=...
                      tstrf(nlidx,fitidx).nltype;
               end
            end
         else
            for nlidx=1:nlcount,
               tokidx=find(~isnan(respforfit(:,nlidx)));
               fitresp=respforfit(tokidx,1);
               
               if std(linpred(tokidx,1))>0,
                  xc0(sfsidx,sigidx,nlidx)=...
                      xcov(linpred(tokidx,1),fitresp,0,'coeff');
                  xc(sfsidx,sigidx,nlidx)=xc0(sfsidx,sigidx,nlidx);
                  xcatt(sfsidx,sigidx,nlidx)=xc0(sfsidx,sigidx,nlidx);
...
if ALLNL,
   for nlidx=1:nlcount,
      ...
      txc=xc(:,:,nlidx);
      if size(txc,1)>size(g,1) & size(txc,2)>=size(g,2),
         txc=rconv2(txc,g);
      end
      ...
      for fitidx=1:attcount-1,
         ...
         th=shrinkage(mH(:,(1:tbincount)-...
                         fparams.maxlag(1),maxsfsidx,1,attidx),...
   ...
   for attidx=1:attcount,
      ...
      if attidx>1 & amaxsfsidx>sfsfit(1,1),
         amaxsfsidx=sfsfit(1,1);
         amaxsigidx=sigfit(1,1);
      end
      
      if attidx==1,
         sfsfit(:,1:(min(localnls)-1))=amaxsfsidx;
         sigfit(:,1:(min(localnls)-1))=amaxsigidx;
         
         expxc(:,1:(min(localnls)-1))=xc(amaxsfsidx,amaxsigidx,attidx);
      else
         sfsfit(attidx-1,localnls)=amaxsfsidx;
         sigfit(attidx-1,localnls)=amaxsigidx;
         
         expxc(attidx-1,localnls)=xc(amaxsfsidx,amaxsigidx,attidx);
      end
      
      % fill strf with best fit kernel according to sigfit and sfsfit
      th=shrinkage(mH(:,(1:tbincount)-...
                      fparams.maxlag(1),amaxsfsidx,1,attidx),...
                   eH(:,(1:tbincount)-fparams.maxlag(1),...
                      amaxsfsidx,1,attidx),...
                   sigrange(amaxsigidx));
      
      
      tstrf1(1,attidx).h=th;
      tstrf1(1,attidx).nlparms=0;
      tstrf1(1,attidx).nltype='none';
      tstrf1(1,attidx).mS=mSall(:,respidx,attidx);
      tstrf1(1,attidx).parms.kernfmt=fparams.kernfmt;
      tstrf1(1,attidx).parms.iconside=fparams.iconside;
      tstrf1(1,attidx).parms.sfsfit=amaxsfsidx;
      tstrf1(1,attidx).parms.sigfit=amaxsigidx;
   ...
   for nlidx=1:nlcount,
      for fitidx=1:attcount-1,
         tstrf(nlidx,fitidx).name=...
             sprintf('%s sfs=%d sig=%d att=%d %s',...
                     cellid,tstrf(nlidx,fitidx).parms.sfsfit,...
                     tstrf(nlidx,fitidx).parms.sigfit,...
                     fitidx,nlnames{nlidx});
      end
   end
...
% fit alternative (cross att conditions!) output nls to support
% randomized attention predictions!  ack!

tstrf=cat(2,strf(1,1),strf(end,:));
for ii=1:attcount,
   tstrf(ii).nltype='none';
end
...
linpred0=linpred;
tstrf=strf;
for masterfitidx=1:attcount-1,
   linpred(:,2:end)=repmat(linpred0(:,masterfitidx+1),[1 attcount-1]);
   
   kvafitnl;
   for strfidx=1:size(strf,1),
      for fffidx=1:size(tstrf,2),
         strf(strfidx,masterfitidx).nlparmsalt{fffidx}=...
             tstrf(strfidx,fffidx).nlparms;
      end
   end
end

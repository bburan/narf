...
smfilt=[zeros(params.maxlag(2)-1,1);ones(params.maxlag(2),1)];
r=conv2(r0,smfilt,'same');

% figure out how to bin the stimulus
sbincount=20;
sbins=linspace(min(s0),max(s0),sbincount+1)';
sbins(end)=inf;
maxr=7;
nevents=zeros(maxr,cellcount);
...
sprior=zeros(sbincount,plagcount);
shist=zeros(sbincount,plagcount,maxr,cellcount);

for ii=1:plagcount,
   thist=histc(stim0(ff+ii+problag(1)-1,:),sbins)+1;
   sprior(:,ii)=thist(1:(end-1));
   sprior(:,ii)=sprior(:,ii)./sum(sprior(:,ii));
end

for r1=1:cellcount,
   for rr=1:maxr,
      if EXCLUDESILENCE,
         ff=find(min(resp(:,r1),maxr-1)==rr-1 & stim0>0);
      else
         ff=find(min(resp(:,r1),maxr-1)==rr-1);
      end
      ff=ff(ff>=problag(2)+1 & ff<size(resp,1)-problag(2));
      nevents(r1,rr)=length(ff);
      if length(ff)>0,
         for ii=1:plagcount,
            thist=histc(stim0(ff+ii+problag(1)-1,:),sbins);
            thist=thist(1:(end-1));
            %if sum(thist)>0,
            shist(:,ii,rr,r1)=thist;
            %else
            %   shist(:,ii)=sprior(:,ii);
            %   disp('0 thist');
            %   keyboard;
         ...
         shist(:,:,rr,r1)=sprior;
         fprintf('no matching events: (cnt,cell)=(%d,%d)\n',rr,r1)
      end
      
   end
   for ii=1:plagcount,
      for ss=1:sbincount
         thist=shist(ss,ii,:,r1);
         dd=find(thist>0);
         shist(ss,ii,:,r1)=-10;
         shist(ss,ii,dd,r1)=log(thist(dd)./sum(thist));
         
         %if sum(thist)>0,
         %   shist(ss,ii,:,r1)=log(thist./sum(thist));
         %else
         %   shist(ss,ii,:,r1)=-10;
         %end
      end
   ...
   %for rr=1:maxr,
   %   shist(:,:,rr,r1)=log(shist(:,:,rr,r1));%-log(sprior);
   %end
end
shist(isinf(shist))=-10; 

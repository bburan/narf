...
% origpix=160;
%
function [hf,stephf,movformat,kern,modspace]=rconline(respfiles,stimfiles,origpix,fmt,params,varargin);
...
% force re-run of processpypedata?
params.forcereload=getparm(params,'forcereload',0);

% smooth before binning?
params.smoothspikes=getparm(params,'smoothspikes',0);

fprintf('%d files to process...\n',length(respfiles));
matfiles={};
for ii=1:length(respfiles),
   matfiles{ii}=processpypedata(respfiles{ii},params.forcereload,...
                                params.smoothspikes);
end
...
% make it world writable so that someone else can re-run
unix('chmod a+wr /tmp/rconline.mat');

% [mH,eH]=xccorestrfpak(stim,resp,params);
%
...
% 
% returns:
% mean and stderr on kernel estimates: mH, eH (X x tbincount x M x sfscount)
% mean and stderr on raw (biased) XC: mSR, eSR (X x tbincount x M x sfscount)
% mean stim/response across resamples: mSall, mRall
%
% params (defaults):
...
%
function [mH,eH]=xccorestrfpak(stim,resp,params);

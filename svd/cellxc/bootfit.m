...
DOTHRESH=0;  % flag do threshold instead of shrinkage
...
if respfmtcode==0,
   if sffiltsigma==1,
      ...
      sigrange=exp(linspace(log(3.0),log(10),sffiltsigma));
...
for attidx=1:attcount,
   ...
   for r=1:respcount,
      ...
      for sfsidx=1:sfscount,
         % smooth the kernel??!?!?!
         thf=mH(:,(1:tbincount)-maxlag(1),sfsidx,r,attidx);
         if resampcount>1,
   ...
   for nlidx=1:nlcount,
      ...
      for latidx=1:latcount,
         if latcount>1,
            thf=mH(:,latidx,sfsfituse,1,attidx);
            ...
            if resampcount>1,
               ...
               if ~DOTHRESH,
                  ...
                  smd=smd.*(smd>0);
                  smd(find(isnan(smd)))=0;
                  hf(:,latidx,nlidx,attidx)=thf.*smd;
               else
                  hf(:,latidx,nlidx,attidx)=thf.*(smd>1);
               end
            else
               hf(:,latidx,nlidx,attidx)=thf;
            end
         else
            thf=mH(:,(1:tbincount)-maxlag(1),sfsfituse,1,attidx);
